...
export default function ProgressPage() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/takes", { credentials: "include" })
      .then((res) => res.json())
      .then((data) => {
        const list = data._embedded?.takesDTOList || [];
        setCourses(list);
        setFilteredCourses(list);
      });
    fetch("http://localhost:8080/api/assessments", { credentials: "include" })
      .then((res) => res.json())
      .then((data) => setAssessments(data._embedded?.assessmentDTOList || []));
    fetch("http://localhost:8080/api/assignments", { credentials: "include" })
      .then((res) => res.json())
      .then((data) => setAssignments(data._embedded?.assignmentDTOList || []));
    fetch("http://localhost:8080/api/submissions", { credentials: "include" })
      .then((res) => res.json())
      .then((data) => setSubmissions(data._embedded?.submissionDTOList || []));
  }, []);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ textAlign: "center", mb: 4, zIndex: 2 }}>
        ...
        <img src={koalaImg} alt="Koala" style={{ width: 100 }} />
        <TextField
          fullWidth
        ...
        >
          ...
          <Box sx={{ display: "flex", gap: 2, mt: 2, flexWrap: "wrap" }}>
            <Button
              ...
              onClick={() =>
                (window.location.href = `/assessments?courseId=${course.courseId}`)
              }
            >
              Assessments
            </Button>
            ...
            <Button
              ...
              onClick={() =>
                (window.location.href = `/assignment?courseId=${course.courseId}`)
              }
            >
              Assignments
            </Button>
