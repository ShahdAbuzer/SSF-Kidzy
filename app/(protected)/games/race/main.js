...
const heighestScoreElement = document.getElementById("hscore");
...
setTimeout(() => {
  ...
  // Removed youtubeLogo style change
...
function reset() {
  ...
  heighestScoreElement.innerText = `Heighest Score : ${localStorage.getItem("highscore") || 0}`;
...
function positionHighScoreElement() {
  const arcCenterXinPixels = (arcCenterX / cameraWidth) * window.innerWidth;
  heighestScoreElement.style.cssText = `
    position: absolute;
    left: ${window.innerWidth / 2 - arcCenterXinPixels * 1.65}px;
    top: ${window.innerHeight / 1.8}px;
    color: white;
...
window.addEventListener("keydown", function (event) {
  if (event.key === "ArrowUp") {
    console.log("ArrowUp pressed, accelerating");
    startGame();
    accelerate = true;
    return;
  }
  if (event.key === "ArrowDown") {
    console.log("ArrowDown pressed, decelerating");
    decelerate = true;
    return;
  }
  if (event.key === "R" || event.key === "r") {
...
function animation(timestamp) {
  ...
  if (laps != score) {
    ...
    heighestScoreElement.innerText = `Heighest Score : ${Math.max(localStorage.getItem("highscore"), score)}`;
  }
...
function getPlayerSpeed() {
  if (accelerate) return speed * 2; // Increase multiplier for testing
  if (decelerate) return speed * 0.2;
  return speed;
}
